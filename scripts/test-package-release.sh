...
scripts/package-release.sh v0.8.0 "$source_commit" "${test_root}/helper" "${test_root}/dropbear" "${test_root}/dropbearkey" "${test_root}/one"
scripts/package-release.sh v0.8.0 "$source_commit" "${test_root}/helper" "${test_root}/dropbear" "${test_root}/dropbearkey" "${test_root}/two"
...
jq -e \
  ...
  '
    ...
    .binary.asset == "liskov-runtime-contact-v0.8.0-aarch64-unknown-linux-musl" and
    .archive.asset == "liskov-runtime-contact-v0.8.0-aarch64-unknown-linux-musl.tar.gz" and
    .toolchain.dropbearVersion == "2026.94" and
    .toolchain.dropbear.asset == "liskov-dropbear-2026.94-v0.8.0-aarch64-unknown-linux-musl" and
    .toolchain.dropbearkey.asset == "liskov-dropbearkey-2026.94-v0.8.0-aarch64-unknown-linux-musl" and
...
if scripts/package-release.sh v0.8.0-rc.1 "$source_commit" "${test_root}/helper" "${test_root}/dropbear" "${test_root}/dropbearkey" "${test_root}/bad-tag"; then
  echo "prerelease tag unexpectedly accepted" >&2
  exit 1
fi
if scripts/package-release.sh v0.8.0 ABCD "${test_root}/helper" "${test_root}/dropbear" "${test_root}/dropbearkey" "${test_root}/bad-commit"; then
